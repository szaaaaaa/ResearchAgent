...
SPEC = SkillSpec(
    skill_id="extract_paper_notes",
    purpose="Analyze corpus sources into structured paper notes.",
    input_artifact_types=["CorpusSnapshot"],
    output_artifact_types=["PaperNote"],
)
...
def handle(input_artifacts: list[Any], cfg: dict[str, Any]) -> SkillResult:
    ...
    if corpus_snapshot is None:
        raise ValueError("extract_paper_notes requires a CorpusSnapshot artifact")
